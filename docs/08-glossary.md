...
| Term | Meaning |
|---|---|
| **The app** / **the client** | `social-commerce-app`, the React Native mobile app — the sibling repo. Referred to as "the client" specifically when contrasting it with this server. |
| **This backend** / **the API** | `social-commerce-backend`, this repo. |
...
| **Session-mode pooler vs. direct connection** | Two different connection strings to the same Supabase Postgres project. The app uses the pooler (`DATABASE_URL`); migrations use the direct connection (`SUPABASE_DIRECT_URL`) because the pooler breaks session-dependent DDL. See [05-deployment-and-operations.md](05-deployment-and-operations.md). |
